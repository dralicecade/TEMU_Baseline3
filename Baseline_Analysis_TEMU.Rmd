...
H_Ego <- ggplot(EGOdf, aes(x = factor(TBI_Non, levels = c("Non", "Short", "Long")), y = Error_X_mm, color = TBI_Non)) + 
  ...
  stat_pvalue_manual(stat.test, label = "p = {scales::pvalue(p.adj)}") + theme(text = element_text(size = 16))   
...
X_OverPlot <- ggplot(PropXUnderTBI  ) + geom_col(aes(x=TBI_Non, y=PropPos), fill=myCols3, alpha = 0.7) + geom_errorbar(aes(x = TBI_Non, ymin = PropPos-CorrSE*1.96, ymax = PropPos + CorrSE*1.96), width=.2, position=position_dodge(.9)) + 
  ...
  theme_minimal() + scale_x_discrete(labels=c("Non" = "Control", "Short" = "mTBI", "Long"= "PPCS")) + ylim(0,1) + theme(text = element_text(size = 16)) 
...
X_OverPlot <- ggplot(PropXUnderTots) + geom_col(aes(x=TBI_Non, y=(PropPos*100)), fill=myCols3LightDark, alpha = 0.7) + geom_errorbar(aes(x = TBI_Non, ymin = (lwr.ci*100), ymax = (upr.ci*100)), width=.2,
  ...
  scale_x_discrete(labels=c("Non" = "Control", "Short" = "mTBI", "Long"= "PPCS")) + theme(text = element_text(size = 16)) 
...
Y_OverPlot <- ggplot(PropYUnderTots) + geom_col(aes(x=TBI_Non, y=(PropPos*100)), fill=myCols3LightDark, alpha = 0.7) + geom_errorbar(aes(x = TBI_Non, ymin = (lwr.ci*100), ymax = (upr.ci*100)), width=.2,
...
T_Pur <- ggplot(PURdf, aes(x = factor(TBI_Non, levels = c("Non", "Short", "Long")), y = TotalVA_Error_Mean_deg, color = TBI_Non)) + 
  ...
  geom_hline(yintercept=0, linetype="dashed", color="#5f6061")+
  stat_pvalue_manual(stat.test, label = "p = {scales::pvalue(p.adj)}")  + theme(text = element_text(size = 12))
...
PT_Sac <- ggbarplot(diffs, x = "TBI_Non", y = "Percentage", 
          color = "TBI_Non",fill = "TBI_Non", palette = myCols3,  alpha = 0.7,  #add = "jitter",
...
AT_Sac <- ggbarplot(diffs, x = "TBI_Non", y = "Percentage", 
          color = "TBI_Non", fill = "TBI_Non", palette = myCols3, alpha = 0.7,
          order = c("Non", "Short", "Long"),
...
stat.test <- stat.test %>%
 mutate(y.position = c(2000, 3000, 4000))
...
H_VOR <- ggplot(VORdf, aes(x = factor(TBI_Non, levels = c("Non", "Short", "Long")), y = HVA_Mean_deg, color = TBI_Non)) + 
  ...
  xlab(" ") + ylab("Horizontal visual angle error (degrees)")+
  geom_hline(yintercept=0, linetype="dashed", color="#5f6061")+
  stat_pvalue_manual(stat.test, label = "p = {scales::pvalue(p.adj)}") + ylim(-6, 6) + theme(text = element_text(size = 12))
...
stat.test <- stat.test %>%
 mutate(y.position = c(1.5,1.65,1.8))
