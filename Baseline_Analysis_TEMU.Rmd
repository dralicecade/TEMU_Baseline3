...
X_OverPlot <- ggplot(PropXUnderTots) + geom_col(aes(x=TBI_Non, y=(PropPos*100)), fill=myCols3LightDark, alpha = 0.7) + geom_errorbar(aes(x = TBI_Non, ymin = (lwr.ci*100), ymax = (upr.ci*100)), width=.2,
  ...
  scale_x_discrete(labels=c("Non" = "Control", "Short" = "mTBI", "Long"= "PCS"))
...
ggsave("FixPlots.png", width = 30, height = 20, units = "cm")
```
...
stat.test <- stat.test %>%
 mutate(y.position = c(1.8, 1.9,2.1))
...
H_VOR <- ggplot(VORdf, aes(x = factor(TBI_Non, levels = c("Non", "Short", "Long")), y = HVA_Mean_deg, color = TBI_Non)) + 
  ...
  xlab(" ") + ylab("Horizontal Visual Angle Error (deg)")+
  geom_hline(yintercept=0, linetype="dashed", color="#5f6061")+
  stat_pvalue_manual(stat.test, label = "p = {scales::pvalue(p.adj)}") + ylim(-6, 6)
